...
class Admin:
    ...
    def admin_mainmenu(self,a,b):
         ...
         mi = PhotoImage(file="images/accounts.png").subsample(a, b).zoom(BUTTON_WIDTH // 90)
         self.mainframe = LabelFrame(self.mainw, width=1200, height=145, bg="#12171c")
         self.mainframe.place(x=100, y=100)
         
         self.accounts = Button(self.mainframe, text="Profiles", font="Montserrat 13 bold", bd=5, image=mi, compound=TOP, command=self.buildusertable, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
         self.accounts.image = mi
         self.accounts.place(x=655, y=27)
         
         self.logout = Button(self.mainframe, text="Quit", bd=5, font="Montserrat 13 bold", image=mi, compound=TOP, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
         self.logout.image = mi
         self.logout.place(x=1050, y=27)
         
         self.changeuser = Button(self.mainframe, text="Sign out", bd=5, font="Montserrat 13 bold", image=mi, compound=TOP, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
         self.changeuser.image = mi
         self.changeuser.place(x=855, y=27)
         
         self.items = Button(self.mainframe, text="Items", bd=5, image=mi, font="Montserrat 13 bold", compound=TOP, command=self.additems, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
         self.items.image = mi
         self.items.place(x=47, y=27)
         
         self.stocks = Button(self.mainframe, text="Inventory", bd=5, image=mi, font="Montserrat 13 bold", compound=TOP, command=self.buildprodtable, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
         self.stocks.image = mi
         self.stocks.place(x=255, y=27)
         
         self.sales = Button(self.mainframe, text="Sales", bd=5, font="Montserrat 13 bold", image=mi, compound=TOP, command=self.buildsalestable, width=BUTTON_WIDTH, height=BUTTON_HEIGHT)
         self.sales.image = mi
         ...
         self.tableframe1 = LabelFrame(self.mainw, width=350, height=700,bg="#12171c", fg="white")
         self.tableframe1.place(x=1200, y=315, anchor=NE)
         self.tableframe1info = self.tableframe1.place_info()
         self.tableframe = LabelFrame(self.mainw, width=350, height=700,bg="#12171c", fg="white")
         self.tableframe.place(x=1300, y=315, anchor=NE)
         self.tableframeinfo=self.tableframe.place_info()
         self.itemframe = Frame(self.mainw, bg="#12171c", width=600, height=300)
         self.itemframe.place(x=420, y=280, anchor=NW)
         self.itemframeinfo=self.itemframe.place_info()
         self.formframe1 = Frame(self.mainw, width=500, height=445, bg="#12171c")
         self.formframe1.place(x=100,y=275)
         self.formframe1info = self.formframe1.place_info()
         self.searchframe = Frame(self.mainw, width=720, height=70, bg="#12171c")
         self.searchframe.place(x=575, y=260)
         self.searchframeinfo = self.searchframe.place_info()
         self.searchbut = Button(self.searchframe, text="Search Description", font="Montserrat 13 bold",bg="#32373d", fg="white", bd=5, command=self.searchprod)
         self.searchbut.place(x=0, y=20, height=40)
         self.searchvar=StringVar()
         self.searchentry = myentry(self.searchframe, textvariable=self.searchvar, font="Montserrat 13", width=25, bg="#12171c",fg="white")
         self.searchentry.place(x=210, y=20, height=40)
         self.printbut = Button(self.searchframe, text="Print", font="Montserrat 13 bold", bd=5, width=8,bg="#32373d", fg="white", command=self.generate_sales_pdf)
         self.printbut.place(x=590, y=20, height=40)
         ...
         self.resetbut = Button(self.searchframe, text="Reset", font="Montserrat 14 bold", bd=5, width=8,bg="#32373d", fg="white", command=self.resetprodtabel)
         self.resetbut.place(x=510, y=18, height=45)
         self.cond=0
         self.buildprodtable()
    ...
    def buildprodtable(self):
         ...
         if(self.cond==1):
            ...
            self.tree.destroy()
         scrollbarx = Scrollbar(self.tableframe, orient=HORIZONTAL, bg="#12171c")
         scrollbary = Scrollbar(self.tableframe, orient=VERTICAL,bg="#12171c")
         ...
         scrollbary.config(command=self.tree.yview,bg="#12171c")
         scrollbarx.grid(row=2, column=0, sticky="we")
         scrollbarx.config(command=self.tree.xview,bg="#12171c")
         ...
         for i in range(0,6):
             Label(self.formframe, text=l1[i], font="Montserrat 13 bold", bg="#12171c", fg="white").place(x=0, y=va)
             va += 60
         Entry(self.formframe, textvariable=self.itemeditv, font="Montserrat 13",bg="#FFFFFF", width=20).place(x=142, y=0, height=30)
         Entry(self.formframe, textvariable=self.itemeditdescv, font="Montserrat 13 ",bg="#FFFFFF", width=20).place(x=142, y=60, height=30)
         x=myentry(self.formframe, textvariable=self.itemeditcatv, font="Montserrat 13 ",bg="#FFFFFF", width=20)
         x.place(x=142, y=120, height=30)
         ...
         Entry(self.formframe, textvariable=self.itemeditpricev, font="Montserrat 13",bg="#FFFFFF", width=20).place(x=142, y=180, height=30)
         Entry(self.formframe, textvariable=self.itemeditstockv, font="Montserrat 13",bg="#FFFFFF", width=20).place(x=142, y=240, height=30)
         Entry(self.formframe, textvariable=self.addstock, font="Montserrat 13",bg="#FFFFFF", width=20).place(x=142, y=300, height=30)
         Button(self.formframe, text="Update", font="Montserrat 11 bold",bg="#32373d", fg="white", bd=5, width=10, height=1,
         command=self.changeprodtable).place(x=105, y=361)
         Button(self.formframe, text="Remove", font="Montserrat 11 bold",bg="#32373d", fg="white", bd=5, width=10, height=1,
    ...
    def additems(self):
        ...
        for i in range(0,len(l)):
            Label(self.itemframe,text=l[i],font="Montserrat 14 bold",bg="#12171c",fg="white").grid(row=i,column=0,pady=15,sticky="w")
        Entry(self.itemframe, width=40, textvariable=self.newitemcode,font="Montserrat 14",bg="white").grid(row=0, column=1,pady=15,padx=10,ipady=3)
        Entry(self.itemframe,width=40,textvariable=self.newitem,font="Montserrat 14",bg="white").grid(row=1,column=1,pady=15,padx=10,ipady=3)
        Entry(self.itemframe,width=40,textvariable=self.newitemdesc,font="Montserrat 14",bg="white").grid(row=2,column=1,pady=15,padx=8,ipady=3)
        cat=myentry(self.itemframe,width=40,textvariable=self.newitemcat,font="Montserrat 14",bg="white")
        cat.grid(row=3,column=1,pady=15,padx=10,ipady=3)
        Entry(self.itemframe, width=40, textvariable=self.newitemprice,font="Montserrat 14",bg="white").grid(row=4, column=1, pady=15, padx=10,ipady=3)
        Entry(self.itemframe, width=40, textvariable=self.newitemstock,font="Montserrat 14",bg="white").grid(row=5, column=1, pady=15, padx=10,ipady=3)
        ...
        Button(self.itemframe,text="Add item",height=1,bd=6,font="Montserrat 14 bold",command=self.insertitem,bg="#32373d", fg="white").grid(row=6,column=1,pady=10,padx=12,sticky="w",ipadx=10)
        Button(self.itemframe, text="Back", height=1,width=8,font="Montserrat 14 bold", bd=6,command=self.buildprodtable,bg="#32373d", fg="white").grid(row=6, column=1,padx=16, pady=10,sticky="e",ipadx=10)
    ...
    def buildusertable(self):
         ...
         for i in range(0,3):
             Label(self.formframe1, text=l1[i], font="Montserrat 13 bold", bg="#12171c", fg="white").place(x=0, y=va)
             va += 70
         Entry(self.formframe1, textvariable=self.usernamedit, font="Montserrat 13", bg="white", width=25).place(x=162, y=105, height=40)
         Entry(self.formframe1, textvariable=self.passwordedit, font="Montserrat 13", bg="white", width=25).place(x=162, y=175, height=40)
         profiles=mycombobox(self.formframe1, font="Montserrat 13", width=23, textvariable=self.accedit)
         profiles.place(x=162,y=245,height=40)
         profiles.set_completion_list(['ADMIN','USER'])
         Button(self.formframe1, text="Create a User", font="Montserrat 13 bold", bg="#32373d", fg="white", bd=5, width=12, height=1,
                command=self.adduser).place(x=0, y=10)
         Button(self.formframe1, text="Update", font="Montserrat 13 bold", bg="#32373d", fg="white", bd=5, width=10, height=1,
                command=self.changeusertable).place(x=145, y=381)
         Button(self.formframe1, text="Remove", font="Montserrat 13 bold", bg="#32373d", fg="white", bd=5, width=10, height=1,
    ...
    def adduser(self):
        if self.usernamedit.get() == '' or self.passwordedit.get() == '' or self.accedit.get() == '':
            messagebox.showerror("Error", "Please Fill All Fields")
            return
        if self.accedit.get() not in ['ADMIN', 'USER']:
            messagebox.showerror("Error", "Unknown account type!")
            return
        
        self.cur.execute("INSERT INTO users (username, password, account_type) VALUES (?, ?, ?);",
                        (self.usernamedit.get().upper(), self.passwordedit.get().upper(), self.accedit.get().upper()))
        self.base.commit()
        self.tree.delete(*self.tree.get_children())
        self.getusers()
        self.usernamedit.set('')
    ...
    def buildsalestable(self):
        ...
        self.getsales()
        self.mainsearch(2)
        self.totalsales=Label(self.tableframe1,text="Total Sales",font="Montserrat 13 bold").place(x=0,y=400)
    ...
    def generate_sales_pdf(self):
        ...
        file_name = os.path.join(dir_path, f"Sales_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.pdf")

        # Define table data and style
        table_data = [["Transaction ID", "Invoice ID", "Product ID", "Quantity", "Date", "Time"]]
        for row in sales_data:
            transID, invoiceID, productID, quantity, date, time = row[0], row[1], row[2], row[3], row[4], row[5]
            table_data.append([transID, invoiceID, productID, quantity, date, time])

        # Create PDF document
        pdf_doc = SimpleDocTemplate(file_name, pagesize=letter)

        # Create table and apply styles
        table = Table(table_data, colWidths=[70, 70, 70, 70, 70, 70])
        table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.black),  # Header row background color (black)
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),  # Header row text color (whitesmoke)
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
            ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),  # Set font to Montserrat-Bold
            ('BOTTOMPADDING', (0, 0), (-1, 0), 12),
            ('BACKGROUND', (0, 1), (-1, -1), colors.grey),  # Content row background color (grey)
            ('GRID', (0, 0), (-1, -1), 1, colors.black)  # Grid color (black)
        ]))


        # Build the PDF document with heading and table
